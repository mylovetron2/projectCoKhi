...
/**
 * Table class for ck_view_nhatky_thietbi
 */
class ck_view_nhatky_thietbi extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'ck_view_nhatky_thietbi';
		$this->TableName = 'ck_view_nhatky_thietbi';
		$this->TableType = 'VIEW';

		// Update Table
		$this->UpdateTable = "`ck_view_nhatky_thietbi`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// thiet_bi_id
		$this->thiet_bi_id = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_thiet_bi_id', 'thiet_bi_id', '`thiet_bi_id`', '`thiet_bi_id`', 3, 11, -1, FALSE, '`thiet_bi_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// chung_loai_id
		$this->chung_loai_id = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_chung_loai_id', 'chung_loai_id', '`chung_loai_id`', '`chung_loai_id`', 3, 11, -1, FALSE, '`EV__chung_loai_id`', TRUE, TRUE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->chung_loai_id->Nullable = FALSE; // NOT NULL field
		$this->chung_loai_id->Required = TRUE; // Required field
		$this->chung_loai_id->Sortable = FALSE; // Allow sort
		...
		// ky_ma_hieu
		$this->ky_ma_hieu = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_ky_ma_hieu', 'ky_ma_hieu', '`ky_ma_hieu`', '`ky_ma_hieu`', 200, 100, -1, FALSE, '`ky_ma_hieu`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->ky_ma_hieu->Nullable = FALSE; // NOT NULL field
		$this->ky_ma_hieu->Required = TRUE; // Required field
		$this->ky_ma_hieu->Sortable = FALSE; // Allow sort
		$this->fields['ky_ma_hieu'] = &$this->ky_ma_hieu;

		// nhan_vien_id
		$this->nhan_vien_id = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_nhan_vien_id', 'nhan_vien_id', '`nhan_vien_id`', '`nhan_vien_id`', 3, 11, -1, FALSE, '`nhan_vien_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->nhan_vien_id->Lookup = new Lookup('nhan_vien_id', 'nhan_vien', TRUE, 'nhan_vien_id', ["danh_so","ten_nhan_vien","",""], [], [], [], [], [], [], '', '');
		$this->nhan_vien_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['nhan_vien_id'] = &$this->nhan_vien_id;

		// ngay_sua_chua
		$this->ngay_sua_chua = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_ngay_sua_chua', 'ngay_sua_chua', '`ngay_sua_chua`', CastDateFieldForLike("`ngay_sua_chua`", 7, "DB"), 133, 10, 7, FALSE, '`ngay_sua_chua`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// noi_dung
		$this->noi_dung = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_noi_dung', 'noi_dung', '`noi_dung`', '`noi_dung`', 201, 1000, -1, FALSE, '`noi_dung`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->noi_dung->Sortable = FALSE; // Allow sort
		$this->fields['noi_dung'] = &$this->noi_dung;

		// ghi_chu
		$this->ghi_chu = new DbField('ck_view_nhatky_thietbi', 'ck_view_nhatky_thietbi', 'x_ghi_chu', 'ghi_chu', '`ghi_chu`', '`ghi_chu`', 201, 500, -1, FALSE, '`ghi_chu`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "ck_danhmuc_thietbi") {
			if ($this->thiet_bi_id->getSessionValue() != "")
				$masterFilter .= "`thiet_bi_id`=" . QuotedValue($this->thiet_bi_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "ck_danhmuc_thietbi") {
			if ($this->thiet_bi_id->getSessionValue() != "")
				$detailFilter .= "`thiet_bi_id`=" . QuotedValue($this->thiet_bi_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_ck_danhmuc_thietbi()
	{
		return "`thiet_bi_id`=@thiet_bi_id@";
	}

	// Detail filter
	public function sqlDetailFilter_ck_danhmuc_thietbi()
	{
		return "`thiet_bi_id`=@thiet_bi_id@";
	}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`ck_view_nhatky_thietbi`";
	}
	...
	public function getSqlSelectList() // Select for List page
	{
		$select = "";
		$select = "SELECT * FROM (" .
			"SELECT *,  FROM `ck_view_nhatky_thietbi`" .
			") `TMP_TABLE`";
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] != "") {
			...
			return "ck_view_nhatky_thietbilist.php";
		}
	}
	...
	{
		global $Language;
		if ($pageName == "ck_view_nhatky_thietbiview.php")
			return $Language->phrase("View");
		elseif ($pageName == "ck_view_nhatky_thietbiedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "ck_view_nhatky_thietbiadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("ck_view_nhatky_thietbiview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("ck_view_nhatky_thietbiview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "ck_view_nhatky_thietbiadd.php?" . $this->getUrlParm($parm);
		else
			$url = "ck_view_nhatky_thietbiadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("ck_view_nhatky_thietbiedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("ck_view_nhatky_thietbiadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("ck_view_nhatky_thietbidelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "ck_danhmuc_thietbi" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
			$url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
			$url .= "&fk_thiet_bi_id=" . urlencode($this->thiet_bi_id->CurrentValue);
		}
		return $url;
	}
